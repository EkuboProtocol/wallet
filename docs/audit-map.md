...
| Endpoint failover never crosses a chain boundary or a plan verdict | `rpc::try_clients` and `rpc::ensure_serving_chain` (every attempt re-verifies the chain ID); `simulation::simulate_execution` retries only on `SimulationFailureCategory::RpcError`; `execution::send_exact_bytes` reconciles per client before moving on; `token_store::nonzero_balances_through` and `batch_read::batch_eth_call_through` re-run whole pinned reads rather than splicing partial ones |
