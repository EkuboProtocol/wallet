...
| Re-simulating a review cannot change what is approved, and signs what was shown | `orchestrator::TransactionReview` (plan, policy, and wallet are borrowed and never replaced; one `latest` slot, replaced per author and taken after the presenter returns), `approve_tui::ReviewScreen::finish_refresh` (a changed document resets `reached_end`, scroll, and the cursor), `approve_tui::run_refresh` (buffered input drained before the new document accepts keys) |
| Agreement across endpoints fails closed | `config::RpcStrategy` (validated against the endpoint count in `config::validate_network`), `rpc::agree_across_endpoints` (a contradiction is refused, an error is a missing witness not a dissenting one), `simulation::SimulationAgreement` (all attempts pinned to one block; compares parent hash, success, gas, output, and balance changes; excludes node-specific wording and locally-derived findings) |
| Endpoint failover never crosses a chain boundary or a plan verdict | `rpc::try_endpoints` and `rpc::ensure_serving_chain` (every attempt re-verifies the chain ID); `simulation::simulate_execution` retries only on `SimulationFailureCategory::RpcError`; `execution::send_exact_bytes` reconciles per endpoint before moving on; `token_store::nonzero_balances_through` and `batch_read::batch_eth_call_through` re-run whole pinned reads rather than splicing partial ones |
| Shipped endpoints are data, not a trust anchor | `networks` (vendored `networks.json`, regenerated by `contrib/rpc-probe`, validated through `config::validate_network` on parse) |
