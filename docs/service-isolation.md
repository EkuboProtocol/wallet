...
- The 1.x line is at 1.8.4 and ships from the `release-1.x` branch. The **Sign and publish release** workflow is v2-only: its tag gate accepts only `v2.*` tags, and it never writes `latest.json` or mutates the 1.x latest release.
...
**Do not delete the 1.x profile or its directory until the move reports completion.** A pending move with a deleted source permanently locks v2 out: the normal resume path re-opens the exact 1.x source files. If the source is already gone, the owner-authorized `complete-without-source` recovery re-verifies destination state and decrypts every destination key against the sealed store, then atomically clears the pending receipt. It requires the same core owner authorization as the move itself, never marks completion on unverified state, and leaves cleanup pending when any destination key is missing or mismatched. A 1.x source below 1.8.2 is refused by the schema gate without modification; upgrade 1.x first and retry.
