<!--
Vendored copy of the GPUI Component design guide.

Source:    https://longbridge.github.io/gpui-component/docs/design-guides
Upstream:  longbridge/gpui-component, website/docs/design-guides.md (branch `main`)
Retrieved: 2026-08-22

Copied verbatim apart from this header, which replaces the upstream site's
Astro frontmatter. The wallet pins gpui-component at rev 26cc9366, which
predates the guide's addition to the repository, so this text is taken from
`main`; re-fetch and diff when the pin moves. Upstream also publishes a
companion Coding Guides page at ../coding-guides.md, which is deliberately not
vendored here — its "one crate per feature" architecture does not describe this
repository, and only the design rules below are normative for the wallet.
-->
