...
- Every wallet-owned persistent setting mutation must terminate in `ekubo-wallet-core`. UI code may collect intent and render results, but it must not write SQLite rows, wallet configuration files, credential-store entries, or wallet security state directly. The agent-file exception is an exact, typed upsert or removal of v2-managed entries: `ekubo_wallet`, containing only the absolute, version-free path of the installed `ekubo-wallet-v2-mcp-bridge` and fixed `--client <harness>` arguments, and hosted companions enumerated by `ekubo_wallet_core::mcp_companions`, containing only their `https://mcp.ekubo.org/mcp/<protocol>` URLs. The local key is shared with 1.x: installing v2 takes the entry over, and `has_wallet_entry` counts only entries executing this build's helper, so automatic sync never silently absorbs a 1.x-connected harness — explicit Sync shows the takeover diff for confirmation. V2 uses its own helper directory. Deselected v2 companions are removed. Claude Desktop receives only the local entry; hosted companions are account-level connectors. No managed entry may contain credentials. The selection itself is persistent core state.
...
## V2 service-isolation implementation

The maintainer requested one feature branch and one PR to main for this work, overriding the ordinary direct-to-main convention. Use the existing `service-isolation` worktree and branch; do not create checkpoint branches. Subagents must have explicit file ownership and share coordinated compile/test checkpoints. Collect compiler diagnostics together, run the relevant tests after integration, and rerun only failures or behaviorally affected tests. Preserve the final workspace/platform gate, but do not dispatch a complete matrix for each small intermediate edit.

V2 uses separate product, data, service, helper and release-channel identities. Fresh setup never reads 1.x state. Only the explicit first-run move may read legacy credentials, and core must verify durable destination custody before exact source-key cleanup. Keep incomplete cleanup resumable before execution leases, including after the old database key is gone. Windows authorization belongs to the protected broker and its isolated native collector; never accept a desktop boolean or caller-selected process as proof. Distinguish native process-boundary tests from actual PIN/biometric interaction. Current scope and evidence are in `docs/service-isolation.md`.
