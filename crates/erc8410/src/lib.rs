#![no_std]
//! ERC-8410 execution plans using only core and alloc.
//! Validation retains the Ekubo wallet's limits and supported capabilities.
...
/// Maximum number of ordered steps accepted by the wallet's plan profile.
/// Producers can check this before constructing a plan.
...
impl DecimalU256 {
    pub fn new(value: impl Into<String>) -> Result<Self> {
        ...
        U256::from_str(&value)
            .map_err(|error| anyhow::anyhow!("decimal quantity must fit uint256: {error}"))?;
        Ok(Self(value))
    }
...
pub enum RevertDecodePlan {
    ErrorResult {
        #[cfg_attr(
            feature = "schema",
            schemars(schema_with = "crate::any_json_array_schema")
        )]
        abi: Vec<Value>,
        #[serde(default)]
        required: bool,
...
pub struct ExecutionPlan {
    ...
    #[cfg_attr(
        feature = "schema",
        schemars(schema_with = "crate::any_json_object_schema")
    )]
...
impl ExecutionPlan {
    /// Parse JSON after checking its byte length, then validate the plan.
    pub fn from_json(input: &str) -> Result<Self> {
        ensure!(
            input.len() <= MAX_SERIALIZED_PLAN_BYTES,
            "execution plan exceeds {MAX_SERIALIZED_PLAN_BYTES} serialized bytes"
        );
        let plan: Self = serde_json::from_str(input).context("invalid execution plan")?;
        plan.validate()?;
        Ok(plan)
    }
    ...
    pub fn digest(&self) -> B256 {
        // Struct field order preserves the wallet's original insertion-ordered
        // JSON digest, regardless of serde_json's preserve_order feature.
        #[derive(Serialize)]
        struct Transaction<'a> {
            chain_id: &'a DecimalU256,
            from: String,
            to: String,
            data: String,
            value: &'a DecimalU256,
        ...
        struct Canonical<'a> {
            ...
            chain_id: &'a DecimalU256,
            sender: String,
            ordered_steps: Vec<DigestStep<'a>>,
        }
...
#[cfg(feature = "schema")]
fn any_json_object_schema(_: &mut schemars::SchemaGenerator) -> schemars::Schema {
