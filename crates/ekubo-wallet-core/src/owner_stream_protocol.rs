...
pub async fn read(stream: &mut (impl AsyncRead + Unpin)) -> Result<Option<Frame>> {
    let Some(bytes) = crate::owner_framing::read_sensitive_frame(stream).await? else {
        return Ok(None);
    };
