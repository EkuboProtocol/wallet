...
/// per-row. `review` groups suggestions by the list that vouched for
...
/// cannot make `review` an unbounded scroll of decisions. It has to
...
impl TokenStore {
    ...
    pub fn propose(
        ...
        // unreadable, which is the same thing. `review` loads the whole
        ...
        for token in tokens {
            ...
            if queued.is_none() {
                    ...
                    "this batch would leave more than {MAX_PENDING_TOKEN_PROPOSALS} tokens \
                     awaiting review; the owner must run `ekubo-wallet review` first"
                );
                pending += 1;
            }
    ...
    pub fn proposals(&self) -> Result<Vec<TokenProposal>> {
        ...
        // it. Changing a name that is already there is what `settings tokens remove`
        // is for.
