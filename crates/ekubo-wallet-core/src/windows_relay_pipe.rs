//! Owner-created, installer-authenticated ciphertext relay channels in a fixed
//! v2 namespace. This module performs no credential lookup.
...
fn name(endpoint: uuid::Uuid) -> Result<String> {
    ...
    Ok(format!(
        r"\\.\pipe\EkuboWalletV2.InstallerRelay.{}",
        endpoint.simple()
    ))
}
