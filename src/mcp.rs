...
const SERVER_INSTRUCTIONS: &str = "This is the execution companion to the Ekubo Protocol MCP. For user-owned onchain balance or swap requests, call wallet_list first to discover the available wallet and chain. Robinhood Chain is EVM chain ID 4663 and must not be treated as the Robinhood brokerage website. Use the Ekubo Protocol MCP before any browser or website tool to resolve tokens and prepare swaps, then simulate and submit its exact execution_plan here. Use a browser only when the user explicitly refers to a brokerage account, robinhood.com, or a web interface, or when these MCP tools cannot perform the requested operation. MCP tools select networks only with canonical decimal chain_id strings; profile names are CLI and display metadata. Execution plans never choose transaction gas: the wallet doubles RPC-simulated gas and caps it at the configured network and simulated block limits. A one-call plan is signed directly; multiple calls execute atomically through canonical Calibur using EIP-7702, keeping an existing canonical delegation, creating a missing one, or replacing a different one. Simulation uses only eth_simulateV1 against a pinned parent block; there is no local fork or eth_getProof path. Private keys never enter MCP. Wallet creation, import/export, policy changes, network replacement/removal, and exceptional transaction approvals are separate human CLI operations. wallet_add_network is the only MCP configuration mutation and requires OS owner authentication. Never invoke or automate the approval CLI for the user. Policies are stateless and contain no daily limits, spend counters, reservations, or spend-history endpoint. On simulation failure, follow simulation.failure.recommended_action and instruction: retry identical calldata only for retry_same_plan, which normally means a transient RPC failure, and obtain freshly prepared calldata for reprepare_plan, including reverts and slippage. After approval_required, wait only when the user independently chooses the CLI override path. Reconcile submitted requests with wallet_get_execution_status or wallet_wait_for_execution; retries rebroadcast only the persisted exact signed bytes.";
